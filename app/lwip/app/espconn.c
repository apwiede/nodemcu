...
//ets_printf("espconn_find_connection: connection pespconn: %p plist->pespconn: %p\n", pespconn, plist->pespconn);
...
//ets_printf("espconn_find_connection: server pespconn: %p plist->pespconn: %p\n", pespconn, plist->pespconn);
    	if (pespconn == plist->pespconn) {
...
ets_printf("espconn_connect called\n");
    if (espconn == NULL) {
        return ESPCONN_ARG;
...
ets_printf("espconn_connect: ESPCONN_AP_STA ? opmode: %d 0x%08x\n", wifi_get_opmode());
    }
...
ets_printf("espconn_connect call espconn_tcp_client\n");
...
ets_printf("espconn_sent: length: %d\n", length);
...
//ets_printf("espconn_sent: value: %p\n", value);
    if (value){
    	espconn ->state = ESPCONN_WRITE;
//ets_printf("espconn_sent: type: %d UDP: %d TCP: %d\n", espconn->type, ESPCONN_UDP, ESPCONN_TCP);
		switch (espconn ->type) {
			case ESPCONN_TCP:
...
ets_printf("espconn_sent ERROR ESPCONN_ARG\n");
    return ESPCONN_ARG;
}
...
ets_printf("espconn_regist_reconcb: \n");
...
ets_printf("espconn_disconnect start\n");
    if (espconn == NULL) {
        return ESPCONN_ARG;;
    } else if (espconn ->type != ESPCONN_TCP)
    	return ESPCONN_ARG;
...
ets_printf("espconn_disconnect end ESPCONN_OK\n");
    	return ESPCONN_OK;
    } else {
ets_printf("espconn_disconnect end ESPCONN_ARG\n");
    	return ESPCONN_ARG;
    }
}
...
ets_printf("espconn_abort start\n");
    if (espconn == NULL) {
        return ESPCONN_ARG;;
    } else if (espconn ->type != ESPCONN_TCP)
    	return ESPCONN_ARG;
