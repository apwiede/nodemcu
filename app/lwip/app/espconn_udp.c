...
ets_printf("espconn_udp_sent: length: %d\n", length);
...
ets_printf("espconn_udp_sent: ERROR ESPCONN_MEM\n");
        return ESPCONN_MEM;
    }
...
ets_printf("espconn_udp_sent: ERROR ESPCONN_ARG1\n");
    		  return ESPCONN_ARG;
    	  }
...
ets_printf("espconn_udp_sent: 3 err: %d\n", err);
        if (err > 0) {
ets_printf("espconn_udp_sent: ERROR ESPCONN_IF\n");
        	return ESPCONN_IF;
        }
        return err;
    } else {
    	pbuf_free(p);
ets_printf("espconn_udp_sent: ERROR ESPCONN_RTE\n");
    	return ESPCONN_RTE;
    }
}
...
//ets_printf("espconn_udp_recv: ip: %d.%d.%d.%d port: %d\n", precv->pcommon.remote_ip[0], precv->pcommon.remote_ip[1], precv->pcommon.remote_ip[2], precv->pcommon.remote_ip[3], port);
    ...
    if (p != NULL) {
        ...
        pbuf_free(p);
		if (length != 0) {
//ets_printf("espconn_udp_recv: recv_callback: %p\n", precv->pespconn->recv_callback);
...
ets_printf("espconn_udp_disconnect\n");
    udp_disconnect(upcb);
...
ets_printf("espconn_udp_server port: %d\n", pespconn->proto.udp->local_port);
    if (upcb == NULL) {
        return ESPCONN_MEM;
    } else {
...
ets_printf("espconn_udp_server end: espconn_udp_recv: %p\n", espconn_udp_recv);
        return ESPCONN_OK;
    }
}
