...
static int file_setencrypt (lua_State *L)
{
  size_t l;
  const char *s = luaL_checklstring(L, 1, &l);
  bool res;
ets_printf("set_encrypt: %s\n", s);
  res = fs_set_encrypt(s);
  if (!res) {
    return luaL_error(L, "Error in setencrypt (key too long) max length: %d", CRYPT_KEY_MAX_LEN);
  } 
  lua_pushboolean(L, 1);
  return 1;
}
...
#endif
#if defined(SPIFFS_USE_ENCRYPT)
  { LSTRKEY( "setencrypt" ), LFUNCVAL( file_setencrypt ) },  
#endif
  { LNILKEY, LNILVAL }
};
