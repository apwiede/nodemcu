...
{
ets_printf("net_server_disconnected is called.\n");
...
{
ets_printf("net_socket_disconnected is called.\n");
...
{
ets_printf("net_server_reconnected is called err: %d.\n", err);
...
{
//ets_printf("net_socket_received is called. %d %s\n", len, pdata);
...
{
ets_printf("net_server_connected is called.\n");
...
ets_printf("call espconn_disconnect\n");
...
{
ets_printf("net_socket_connected is called.\n");
...
ets_printf("connection: secure: %d\n", secure);
      stack++;
      if ( secure != 0 && secure != 1 ){
        return luaL_error( L, "wrong arg type" );
      }
    } else {
ets_printf("connection: secure no number\n");
...
#endif
    {
ets_printf("socket_connect called\n");
...
static int net_start( lua_State* L, const char* mt )
{
  ...
  if( pesp_conn->type == ESPCONN_TCP )
  {
    if(isserver){   // no secure server support for now
     int res =  espconn_regist_connectcb(pesp_conn, net_server_connected);
...
ets_printf("socket: test secure:\n");
#ifdef CLIENT_SSL_ENABLE
ets_printf("socket: secure: %d\n", nud->secure);
      if(nud->secure){
      	if(pesp_conn->proto.tcp->remote_port || pesp_conn->proto.tcp->local_port)
ets_printf("socket: secure: %d call espconn_secure_disconnect\n", nud->secure);
  ...
  {
ets_printf("udp net_start: recvcb: %p isserver: %d\n", net_socket_received, isserver);
    espconn_regist_recvcb(pesp_conn, net_socket_received);
ets_printf("udp net_start: regist net_socket_received: %p\n", net_socket_received);
    espconn_regist_sentcb(pesp_conn, net_socket_sent);
ets_printf("udp net_start: regist net_socket_sent: %p\n", net_socket_sent);
  	if(pesp_conn->proto.tcp->remote_port || pesp_conn->proto.tcp->local_port)
    	espconn_delete(pesp_conn);
    if(isserver) {
ets_printf("udp net_start: espconn_create: pesp_conn: %p\n", pesp_conn);
...
#ifdef CLIENT_SSL_ENABLE
  if(nud->secure) {
ets_printf("sec payload to send: %s\n", payload);
