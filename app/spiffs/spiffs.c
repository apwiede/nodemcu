...
static bool myspiffs_set_location(spiffs_config *cfg, int align, int offset, int block_size) {
  ...
  free_block_res = ( u32_t )platform_flash_get_first_free_block_address( &block_no ) + offset; 
//ets_printf("first_free_block: 0x%08x block_no: %d\n", free_block_res, block_no);
  cfg->phys_addr = SPIFFS_FIXED_OFFSET_RBOOT + (bootconf.roms[bootconf.current_rom] - ((BOOT_CONFIG_SECTOR + 1) * SECTOR_SIZE));
...
//ets_printf("set_location: size: 0x%08x block_size: 0x%08x phys_addr: 0x%08x\n", cfg->phys_size, block_size, cfg->phys_addr);
